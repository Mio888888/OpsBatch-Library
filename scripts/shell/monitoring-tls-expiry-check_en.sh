...
if [[ -z "${cert_text}" ]]; then
  echo "CRITICAL - tls ${TARGET_HOST}:${TARGET_PORT} certificate could not be read"
  exit 2
fi
...
echo "${status} - tls ${TARGET_HOST}:${TARGET_PORT} expires_in=${days_left}d warn=${WARN_DAYS}d crit=${CRIT_DAYS}d"
