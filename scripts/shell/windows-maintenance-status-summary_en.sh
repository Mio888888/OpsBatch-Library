...
if [[ ! "${MAX_EVENTS}" =~ ^[1-9][0-9]*$ ]]; then
  echo "MAX_EVENTS must be a positive integer." >&2
  exit 2
fi
...
if [[ -z "${POWERSHELL}" ]]; then
  echo "PowerShell executable not found. Run this shell script from a Windows host with pwsh or Windows PowerShell available." >&2
  exit 1
fi

echo "Windows maintenance status summary"
echo "Days back: ${DAYS_BACK}"
echo "Max events: ${MAX_EVENTS}"
echo "This script is read-only and does not install updates, clear logs, restart services, or modify tasks."
echo
