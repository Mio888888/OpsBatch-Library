...
if [[ -z "${status_code}" || "${status_code}" == "000" ]]; then
  echo "CRITICAL - HTTP request failed for ${TARGET_URL} using ${method} within ${TIMEOUT_SECONDS}s"
  exit 2
fi
...
echo "${status} - http ${TARGET_URL} status=${status_code} latency=${elapsed_ms}ms (${reason})"
echo "Details: method=${method}; timeout=${TIMEOUT_SECONDS}s; response body and headers are not printed."
exit "${exit_code}"
