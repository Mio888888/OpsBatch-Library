...
echo "${status} - memory_used=${mem_used_pct}% swap_used=${swap_used_pct}% (${reason_text})"
echo "Memory detail: ${mem_detail}"
echo "Swap detail: ${swap_detail:-not available or not configured}"
exit "${exit_code}"
