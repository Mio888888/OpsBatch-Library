...
if [[ -z "${answers}" ]]; then
  echo "CRITICAL - dns ${TARGET_HOST} returned no A records using ${method}"
  exit 2
fi
...
echo "${status} - dns ${TARGET_HOST} answers=$(printf '%s' "${answers}" | wc -l | tr -d ' ') latency=${elapsed_ms}ms (${reason})"
echo "Resolver: ${DNS_SERVER:-system default}; method=${method}; timeout=${TIMEOUT_SECONDS}s"
echo "Sample answers:"
printf '%s\n' "${answers}"
exit "${exit_code}"
