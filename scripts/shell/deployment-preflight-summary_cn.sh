...
echo "信息：Deployment preflight summary"
...
echo "信息：Artifact path: ${ARTIFACT_PATH:-<not provided>}"
echo "信息：Checksum file: ${CHECKSUM_FILE:-<not provided>}"
...
echo "信息：This script is read-only and does not deploy, restart, migrate, or switch symlinks."
echo

echo "信息：== Required tool availability =="
...
echo "信息：== Artifact checksum preview =="
if [[ -n "${ARTIFACT_PATH}" && -f "${ARTIFACT_PATH}" ]]; then
  ...
  echo "信息：No artifact file provided for digest preview."
fi
if [[ -n "${CHECKSUM_FILE}" ]]; then
  if [[ -f "${CHECKSUM_FILE}" ]]; then
    echo "信息：Checksum file exists; run protected deploy or dedicated verification with operator approval."
  else
    echo "信息：Checksum file path was provided but does not exist."
  fi
fi
echo

echo "信息：== Release directory summary =="
if [[ -n "${RELEASES_DIR}" && -d "${RELEASES_DIR}" ]]; then
  ...
  echo "信息：No releases directory provided or directory missing."
fi
...
echo "信息：== Optional validation command =="
if [[ -n "${VALIDATE_CMD}" ]]; then
  echo "信息：Running read-only validation command supplied by operator: ${VALIDATE_CMD}"
  bash -c "${VALIDATE_CMD}"
else
  echo "信息：No VALIDATE_CMD provided."
fi
echo

echo "信息：== Optional health URL probe =="
