...
if [ -z "$CACHE_TARGET" ]; then
  echo "Refusing to run: set CACHE_TARGET explicitly, for example CACHE_TARGET=apt, CACHE_TARGET=dnf, CACHE_TARGET=pacman, CACHE_TARGET=brew, or CACHE_TARGET=all."
  exit 0
fi

echo "== protected package cache cleanup plan =="
...
case "$CACHE_TARGET" in
  ...
  brew) brew cleanup -s ;;
  *) echo "Unsupported CACHE_TARGET: $CACHE_TARGET. Supported values: all, apt, dnf, yum, pacman, zypper, brew." ;;
esac
