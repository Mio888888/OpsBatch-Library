...
if [ -z "$TARGET_PACKAGE" ]; then
  echo "Refusing to run: set TARGET_PACKAGE explicitly. Use TARGET_PACKAGE=all only after reviewing full-system upgrade impact."
  exit 0
fi
...
echo "== protected package upgrade plan =="
...
case "$PACKAGE_MANAGER:$TARGET_PACKAGE" in
  ...
  pacman:all) echo "pacman full upgrade would run: sudo pacman -Syu" ;;
...
if [ "$CONFIRM_UPGRADE" != "UPGRADE_PACKAGE" ]; then
  echo
  echo "Dry-run only. Set CONFIRM_UPGRADE=UPGRADE_PACKAGE after reviewing changelog, dependencies, maintenance window, backups, and rollback plan."
  exit 0
fi
