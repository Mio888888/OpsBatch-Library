...
if [ "$(uname -s)" = "Linux" ]; then
  ...
  if command -v lsns >/dev/null 2>&1; then
    ...
    echo "信息：lsns not installed; showing namespace symlinks for first processes."
  ...
  if command -v ip >/dev/null 2>&1; then
    echo
    echo "信息：== named network namespaces =="
    ip netns list 2>/dev/null || true
  fi
else
  echo "信息：Network namespaces are Linux-specific; no macOS equivalent is shown."
fi
