...
echo "信息：== recent warning/error logs since: $SINCE =="
if [ "$(uname -s)" = "Linux" ] && command -v journalctl >/dev/null 2>&1; then
  journalctl --since "$SINCE" -p warning..alert -n "$LINES" --no-pager 2>/dev/null || echo "信息：journalctl query failed; check permissions or time expression."
  ...
  echo "信息：macOS fallback uses --last 1h by default; adjust manually for precise SINCE expressions."
  ...
  echo "未找到受支持的 system log source found.（No supported system log source found.）"
fi
