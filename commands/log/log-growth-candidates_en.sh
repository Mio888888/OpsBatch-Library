...
echo "== log growth candidates =="
echo "Directory: $TARGET_LOG_DIR"
...
if find "$TARGET_LOG_DIR" -xdev -maxdepth "$MAX_DEPTH" -type f -name '*.log' -size +"$MIN_SIZE" -print >/dev/null 2>&1; then
  ...
  echo "This platform's find may not support -maxdepth or size suffix $MIN_SIZE; listing largest log-like files instead."
...
echo
echo "This command only lists candidates and does not modify logs."
