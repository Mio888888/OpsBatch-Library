...
echo "信息：== old log cleanup candidates under $TARGET_LOG_DIR older than $OLDER_THAN_DAYS days =="
...
if [ "$CONFIRM_DELETE" != "DELETE_OLD_LOGS" ]; then
  echo
  echo "仅试运行。 请设置 CONFIRM_DELETE=DELETE_OLD_LOGS to interactively delete matched old log files after reviewing retention and audit requirements.（Dry-run only. Set CONFIRM_DELETE=DELETE_OLD_LOGS to interactively delete matched old log files after reviewing retention and audit requirements.）"
  exit 0
fi

echo
echo "信息：Deleting matched old logs interactively. Answer each prompt carefully."
find "$TARGET_LOG_DIR" -xdev -type f \
  \( -name '*.log.*' -o -name '*.gz' -o -name '*.zip' -o -name '*.xz' -o -name '*.bz2' -o -name '*.old' \) \
