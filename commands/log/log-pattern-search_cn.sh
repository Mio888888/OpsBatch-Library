...
echo "信息：== search pattern in $LOG_FILE =="
echo "信息：Pattern: $LOG_PATTERN"
grep -Ein "$LOG_PATTERN" "$LOG_FILE" 2>/dev/null | tail -n "$LINES" || echo "信息：No matches found or file cannot be read."
