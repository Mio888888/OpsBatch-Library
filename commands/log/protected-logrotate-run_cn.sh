...
if [ "$CONFIRM_ROTATE" != "RUN_LOGROTATE" ]; then
  echo
  echo "仅试运行。 请设置 CONFIRM_ROTATE=RUN_LOGROTATE and LOGROTATE_CONFIG explicitly to execute logrotate after reviewing dry-run output and maintenance window.（Dry-run only. Set CONFIRM_ROTATE=RUN_LOGROTATE and LOGROTATE_CONFIG explicitly to execute logrotate after reviewing dry-run output and maintenance window.）"
  exit 0
fi

if [ "$LOGROTATE_CONFIG_WAS_SET" != "true" ]; then
  echo
  echo "信息：Refusing real run: set LOGROTATE_CONFIG explicitly together with CONFIRM_ROTATE=RUN_LOGROTATE."
  exit 0
fi

echo
echo "信息：Executing logrotate with config $LOGROTATE_CONFIG."
if [ -n "$LOGROTATE_STATE" ]; then
