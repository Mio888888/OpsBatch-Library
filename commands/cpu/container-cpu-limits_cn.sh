...
if [ "$(uname -s)" = "Linux" ]; then
  echo "信息：== cgroup v2 CPU limit =="
  if [ -r /sys/fs/cgroup/cpu.max ]; then
    cat /sys/fs/cgroup/cpu.max
    echo "信息：Format: <quota> <period>; max means unlimited."
  else
    echo "cgroup v2 cpu.max 未找到.（cgroup v2 cpu.max not found.）"
  fi

  echo
  echo "信息：== cgroup v1 CPU limit =="
  if [ -r /sys/fs/cgroup/cpu/cpu.cfs_quota_us ] || [ -r /sys/fs/cgroup/cpu,cpuacct/cpu.cfs_quota_us ]; then
    ...
    echo "cgroup v1 CPU quota files 未找到.（cgroup v1 CPU quota files not found.）"
  fi
  ...
  echo "信息：macOS does not use Linux cgroup CPU limits."
  sysctl hw.ncpu hw.physicalcpu hw.logicalcpu 2>/dev/null || true
else
  echo "未找到受支持的 cgroup CPU command found.（No supported cgroup CPU command found.）"
fi
