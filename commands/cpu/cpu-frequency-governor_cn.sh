...
if [ "$(uname -s)" = "Linux" ]; then
  if command -v cpupower >/dev/null 2>&1; then
    ...
    if ls /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor >/dev/null 2>&1; then
      ...
      echo "信息：No cpufreq governor files found."
    fi

    echo
    echo "信息：== Current frequency =="
    if ls /sys/devices/system/cpu/cpu*/cpufreq/scaling_cur_freq >/dev/null 2>&1; then
      ...
      echo "信息：No CPU frequency source found."
    fi
  fi
elif [ "$(uname -s)" = "Darwin" ]; then
  sysctl hw.cpufrequency hw.cpufrequency_min hw.cpufrequency_max 2>/dev/null || true
  echo "信息：Apple Silicon may not expose fixed CPU frequency via sysctl."
else
  echo "未找到受支持的 CPU frequency command found.（No supported CPU frequency command found.）"
fi
