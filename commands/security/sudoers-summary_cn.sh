...
command -v sudo >/dev/null 2>&1 && sudo -V 2>/dev/null | head -5 || echo "sudo command 未找到 or 不可读."
...
if [ -n "$TARGET_USER" ]; then
  ...
  sudo -l -U "$TARGET_USER" 2>/dev/null || echo "无法列出 $TARGET_USER 的 sudo 权限；可能需要权限。"
else
  echo
  echo "请设置 TARGET_USER，以便对单个用户运行 sudo -l -U。"
fi
