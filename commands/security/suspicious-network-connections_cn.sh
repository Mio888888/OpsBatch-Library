...
echo "信息：== established remote connection counts =="
if command -v ss >/dev/null 2>&1; then
  ...
  echo "未找到受支持的 connection listing tool found.（No supported connection listing tool found.）"
fi

echo
echo "信息：== connections on uncommon high-risk service ports =="
if command -v ss >/dev/null 2>&1; then
...
echo
echo "信息：Review only: high counts or sensitive service ports are investigation leads, not proof of compromise."
