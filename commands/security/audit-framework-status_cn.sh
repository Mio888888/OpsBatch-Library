...
if [ "$(uname -s)" = "Linux" ]; then
  if command -v auditctl >/dev/null 2>&1; then
    ...
    echo "auditctl 未找到.（auditctl not found.）"
  fi

  if command -v ausearch >/dev/null 2>&1; then
    echo
    echo "信息：== recent audit anomalies =="
  ...
  echo "不支持的平台 for audit framework status.（Unsupported platform for audit framework status.）"
fi
