...
echo "== processes running from temporary or user-writable paths =="
...
echo
echo "== deleted executable or library hints on Linux =="
if [ "$(uname -s)" = "Linux" ] && command -v lsof >/dev/null 2>&1; then
  lsof +L1 2>/dev/null | head -n "$LIMIT" || true
else
  echo "Deleted-open-file inspection requires Linux with lsof."
fi

echo
echo "== high privilege long-running processes =="
if [ "$(uname -s)" = "Linux" ]; then
