...
echo "正在显示 top $limit processes by thread count. Override with PROCESS_LIMIT=<n> if needed.（Showing top $limit processes by thread count. Override with PROCESS_LIMIT=<n> if needed.）"
...
if [ "$(uname -s)" = "Linux" ] && ps -eo pid,ppid,user,nlwp,stat,%cpu,%mem,comm --sort=-nlwp >/dev/null 2>&1; then
  ...
  if command -v top >/dev/null 2>&1; then
    ...
    echo "未找到受支持的 thread count command found on macOS.（No supported thread count command found on macOS.）"
  fi
else
