...
if [ -z "$TARGET_DEVICE" ]; then
  echo "请设置 TARGET_DEVICE before running, for example /dev/sda or disk0.（Set TARGET_DEVICE before running, for example /dev/sda or disk0.）"
...
if command -v smartctl >/dev/null 2>&1; then
  ...
  echo "信息：smartctl not installed. Install smartmontools to check SMART health."
fi
