...
if [ -z "$TARGET_USER" ] || [ -z "$TARGET_DIR" ]; then
  echo "拒绝执行： set TARGET_USER and TARGET_DIR explicitly, for example TARGET_USER=app TARGET_DIR=/var/www.（Refusing to run: set TARGET_USER and TARGET_DIR explicitly, for example TARGET_USER=app TARGET_DIR=/var/www.）"
  exit 0
fi
...
echo "信息：== files owned by $TARGET_USER under $TARGET_DIR (max depth $MAX_DEPTH) =="
