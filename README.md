...
├── scripts/                     # 运维脚本
│   ├── shell/                   # Shell 脚本及 .meta.json（可在元数据中标记 inspection/security/deployment/monitoring 等分类）
...
- 高风险处置示例必须默认只输出计划或拒绝执行，要求显式目标变量和确认变量后才执行真实变更。
